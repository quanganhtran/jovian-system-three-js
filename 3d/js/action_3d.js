...
function animate() {
    ...
    ioLine.position.copy(io.mesh.position);
    controls.update();
}
...
$(document).ready(
    function() {
        ...
        */
        var materialFlare = new THREE.MeshBasicMaterial({map: textureFlare});
        materialFlare.transparent = true;
        //materialFlare.blending = THREE.AdditiveBlending;
        //materialFlare.alphaMap = THREE.ImageUtils.loadTexture( "textures/lensflare/lensflare_alpha.png" );
        var geometryFlare = new THREE.PlaneBufferGeometry(400, 400, 30, 30);
        sun = new THREE.Mesh(geometryFlare, materialFlare);
        sun.position.z = 2800;
        //scene.add(sun);
        ...
        //sprite.position.set( -100, 50, 100 );
        sprite.position.z = 2800;
        sprite.scale.set( 400, 400, 1.0 );
        scene.add( sprite );

        //addLight( 0.55, 0.9, 0.5, 200, 0, -200 );
        /*
        function addLight( h, s, l, x, y, z ) {

            var light = new THREE.PointLight( 0xffffff, 1.5, 2500 );
        ...
        */
        ...
        ioLine = new THREE.Line( lineGeo, lineMat );
        scene.add( ioLine );

        /*var geometry = new THREE.RingGeometry( 99, 100, 32 );
...
/* Used during design only
window.addEventListener('keypress',this.readKey,false);
function readKey(e) {
    //alert(e.keyCode);
    switch (e.keyCode) {
        case 97:
...
function changeTurn(axis, amount) {
    switch (axis) {
        case "x":
            milkyWay.mesh.rotation.x += amount;
            break;
        case "y":
            milkyWay.mesh.rotation.y += amount;
            break;
        case "z":
            milkyWay.mesh.rotation.z += amount;
            break;
    }
}
*/